...
def add_supply_to_winter_holidays(holiday_hash, supply)
  ...
  holiday_hash[:winter].values.collect {|holiday| holiday << supply}
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.keys.each do |season|
    ...
    holiday_hash[season].keys.each do |holiday|
      holiday_line = ""
      holiday_start = "  " + holiday.to_s.split("_").collect{|word|word.capitalize}.join(" ") + ": "
      holiday_line << holiday_start
      holiday_hash[season][holiday].each do |supply|
        holiday_line << supply + ", "
      end
      puts holiday_line[0..-3]
    end 
  end
end
